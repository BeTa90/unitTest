...
module.exports = {
  ...
  forwardIasToken: function (serviceCredentials){
  ...
  getDestinations(req, function(err,destinationsData) {
    ...
    destinationsData.forEach(function (destination) {
      if (destination.hasOwnProperty('sap.cloud.service')) {
        ...
        } else if (destination.hasOwnProperty('domain') && !destination.hasOwnProperty('uaaDomain') && !destination.hasOwnProperty('xsappname')) {
          iasDestinations = addIasDestination(destination, iasDestinations);
        } else {
    ...
    addIasData(destinationBSCredentials,iasDestinations);
...
function getDestinations(req,cb){
  if (req.destinationKey) {
    destinationUtils.getDestinationsByDestinationKey(req.app, req.destinationKey,true, function (err, instanceDestinations) {
      if (err) {
        return cb(err);
      }
...
function addIasDestination(destination, iasDestinations){
  if (!iasDestinations) {
    iasDestinations = {};
  }
  iasDestinations[destination['sap.cloud.service']] = {
...
function addIasData(destinationBSCredentials, iasDestinations){
  if (!iasDestinations || !destinationBSCredentials){
    return;
  }
  for (let sapCloudService in iasDestinations) {
    if (destinationBSCredentials[sapCloudService] && destinationBSCredentials[sapCloudService].credentials) {
      destinationBSCredentials[sapCloudService].credentials['ias'] = iasDestinations[sapCloudService];
    }
  }
}
